...
async function providerCall<T>(context: ResearchToolContext, detail: string, operation: () => Promise<T>) {
  context.calls.take();
...
const financialData = (context: ResearchToolContext) => tool({
  ...
  execute: async input => citedProviderResult(await providerCall(context, `${input.ticker} ${input.dataset}`, () => financialDatasetsProvider.financials(input)), context),
});
...
const secFilings = (context: ResearchToolContext) => tool({
  ...
  execute: async input => citedProviderResult(await providerCall(context, `${input.ticker} SEC filings`, () => secEdgarProvider.filings(input)), context),
});
...
const marketQuote = (context: ResearchToolContext) => tool({
  ...
  execute: async ({ ticker }) => citedProviderResult(await providerCall(context, `${ticker} market quote`, () => alphaVantageProvider.quote(ticker)), context),
});
...
const marketOverview = (context: ResearchToolContext) => tool({
  ...
  execute: async () => citedProviderResult(await providerCall(context, "Market overview", () => alphaVantageProvider.overview()), context),
});
...
const weatherForecast = (context: ResearchToolContext) => tool({
  ...
  execute: async ({ place, start }) => citedProviderResult(await providerCall(context, `Weather for ${place}`, () => openWeatherProvider.forecast(place, start)), context),
});
...
const places = (context: ResearchToolContext) => tool({
  ...
  execute: async input => {
    const result = await providerCall(context, `Places near ${input.destination}`, () => googlePlacesProvider.places(input.destination, input.interests));
    return citedProviderResult(result, context);
  },
});

const events = (context: ResearchToolContext) => tool({
  description: "Find date-bounded Ticketmaster events at a destination. Use ISO date-times with offsets and an optional narrow keyword.",
  ...
  execute: async input => citedProviderResult(await providerCall(context, `Events in ${input.destination}`, () => ticketmasterProvider.events(input.destination, input.start, input.end, input.keyword)), context),
});
...
const routes = (context: ResearchToolContext) => tool({
  ...
  execute: async input => citedProviderResult(await providerCall(context, `${input.mode} route to ${input.destination}`, () => googleRoutesProvider.routes(input)), context),
});
...
const travelRequirements = (context: ResearchToolContext) => tool({
  ...
  execute: async input => citedProviderResult(await providerCall(context, `Travel requirements for ${input.destination}`, () => sherpaProvider.requirements(input)), context),
});
