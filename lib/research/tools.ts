...
const webSearch = (context: ResearchToolContext) => tool({
  ...
  execute: async input => {
    context.calls.take(); const limit = context.evidence.reserve(input.limit);
    ...
    return { results: sources.map(source => ({ sourceId: source.id, title: source.title, url: source.canonicalUrl, publisher: source.publisher, publishedAt: source.publishedAt, retrievedAt: source.retrievedAt, excerpt: source.excerpt })), limitation: sources.length < limit ? "Some results were duplicates or invalid and were omitted." : undefined };
  },
});
